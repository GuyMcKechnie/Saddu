import datetime, nextcord, random
      ...
      async def rules(self, act: Interaction):
            if act.user.guild_permissions.administrator:
                  ...
                  rulesEmbed.add_field(name="3. Spamming",
                                    ...
                                    inline=False)
                  rulesEmbed.add_field(name="4. Swearing", value="Swearing is allowed but keep it to a low.", inline=False)
      ...
      async def members(self, act: Interaction):
            if act.user.guild_permissions.administrator:
                  ...
                  membersEmbed = Embed(title="❈ __Sicarius CTF Members__ ❈",
                                    ...
                                    timestamp=datetime.datetime.utcnow())
                  membersEmbed.add_field(name=f"**Tier 1 Members**", value=f">>> {tierOnes}", inline=False)
                  membersEmbed.add_field(name=f"**Tier 2 Members**", value=f">>> {tierTwos}", inline=False)
                  membersEmbed.add_field(name=f"**Tier 3 Members**", value=f">>> {tierThrees}", inline=False)
                  membersEmbed.add_field(name=f"**Tier 4 Members**", value=f">>> {tierFours}", inline=False)
                  membersEmbed.add_field(name=f"\n**__Scrim Participants__**", value=f">>> {scrimParts}", inline=False)
      ...
      async def features(self, act: Interaction):
            ...
            if act.user.guild_permissions.administrator:
                  ...
                  features = [("__Levelling System__:", "A fully functional levelling system to replace the existing levelling system. Features that will be included should replace and update the existing levelling system."),
                              ("__Better Music System__:", "A better functioning music system. Features that will be included:\n> Spotify Integration."),
                              ...
                              ("__Reaction Roles__:", "A sophisticated reaction roles to replace the existing reaction roles."),
