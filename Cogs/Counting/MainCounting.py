    ...
    async def on_message(self, message: Message):
        if message.channel.id == 964947678905659442 and not message.author.bot:
            try:
                ...
                countingChannel = self.client.get_channel(964947678905659442) # Counting channel
                ...
                # Author ID 
                fl = open("/home/container/Cogs/Counting/AuthorCountingDatabase.json")
                flData = json.load(fl)
                authorID = int(flData[0])
                
                # Check points
                cpFile = open("/home/container/Cogs/Counting/CheckPoints.json", 'r')
                ...
                if not authorID == message.author.id: # If author is not previous counter
                    if num == number+1: # Correct number
                        ...
                        authorIDUpdate = [message.author.id] # Updating author ID
                        ...
                        if checkPoint > 0:
                            ...
                            with open("/home/container/Cogs/Counting/CountingDatabase.json", "w") as outfile:
                                json.dump(numberDict, outfile)
                            authorIDUpdate = [925934213029589062] # Updating author ID
                            with open("/home/container/Cogs/Counting/AuthorCountingDatabase.json", "w") as outfile:
                                json.dump(authorIDUpdate, outfile)
                        ...
                        loggingChannel = self.client.get_channel(955776300776300594)
